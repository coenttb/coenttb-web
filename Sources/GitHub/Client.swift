...
public struct Client: Sendable {
    ...
    public var fetchAuthToken: @Sendable (_ code: String) async throws -> Either<OAuthError, AccessToken>

    /// Fetches a GitHub user's emails.
    public var fetchEmails: @Sendable (_ accessToken: AccessToken) async throws -> [GitHubUser.Email]

    /// Fetches a GitHub user from an access token.
    public var fetchUser: @Sendable (_ accessToken: AccessToken) async throws -> GitHubUser
}
...
extension Client: TestDependencyKey {
    public static let testValue: Client? = Client()
}
